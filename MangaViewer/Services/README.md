...
- OCR
  - `OcrService`: singleton OCR pipeline with `Hybrid` and `Vlm` backends, language/grouping/writing settings, paragraph gap tuning, Ollama model settings, ONNX execution-provider options, adjacent-page prefetch controls, and OCR result caching.
...
- Translation
  ...
  - `TranslationClientFactory`: builds the active `IChatClient` for Google, OpenAI, Anthropic, or Ollama.
...
- OCR and translation
  ...
  - If translation is enabled, `TranslationClientFactory` builds the active chat client from `TranslationSettingsService`, then translated text is applied back onto OCR boxes and overlay text.
...
Extension points
- Add a new translation provider by extending `TranslationProviderKind`, `TranslationProviders`, `TranslationSettingsService`, and `TranslationClientFactory`.
